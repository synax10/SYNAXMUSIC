...
from PrachiMusic.modules.main.database import get_loop
...
async def skip(cli, message: Message, _, chat_id):
    ...
    if "live_" in queued:
        ...
        button = telegram_markup(_, videoid, chat_id)
        ...
        try:
            ...
            image = None
        try:
            await Kaal.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await Kaal.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
