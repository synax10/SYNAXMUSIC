...
from PrachiMusic.utilities.strings import get_string
from PrachiMusic.utilities.config import (AUTO_DOWNLOADS_CLEAR, BANNED_USERS,
...
from PrachiMusic.misc import SUDOERS, db
from PrachiMusic.modules.utils.sys import bot_sys_stats
from PrachiMusic.modules.main.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from PrachiMusic.modules.utils.formatters import seconds_to_min
from PrachiMusic.utilities.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from PrachiMusic.modules.main.streamer.autoclear import auto_clean
from PrachiMusic.modules.utils.thumbnails import gen_thumb

wrong = {}
checker = {}

@bot.on_callback_query(filters.regex("MainMarkup") & ~BANNED_USERS)
@languageCB
...
@bot.on_callback_query(filters.regex("unban_assistant"))
async def unban_assistant_(_, CallbackQuery):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    chat_id, user_id = callback_request.split("|")
    a = await bot.get_chat_member(int(chat_id), bot.id)
    if not a.can_restrict_members:
        ...
        try:
            await bot.unban_chat_member(int(chat_id), int(user_id))
        except:
            return await CallbackQuery.answer(
...
@bot.on_callback_query(filters.regex("ADMIN") & ~BANNED_USERS)
@languageCB
async def del_back_playlist(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    ...
    if command == "Pause":
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Kaal.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Kaal.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Kaal.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
