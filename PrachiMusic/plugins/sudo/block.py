...
from PrachiMusic.modules.main.database import add_gban_user, remove_gban_user
...
@bot.on_message(filters.command(BLOCK_COMMAND) & SUDOERS)
@language
...
@bot.on_message(filters.command(UNBLOCK_COMMAND) & SUDOERS)
@language
...
@bot.on_message(filters.command(BLOCKED_COMMAND) & SUDOERS)
@language
async def sudoers_list(client, message: Message, _):
    ...
    for users in BANNED_USERS:
        try:
            user = await bot.get_users(users)
            user = (
