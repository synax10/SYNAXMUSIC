...
from PrachiMusic.utilities.config import adminlist, chatstats, clean, userstats
from PrachiMusic.utilities.strings import get_command
from PrachiMusic import app, bot
...
from PrachiMusic.modules.utils.formatters import alpha_to_int
...
@bot.on_raw_update(group=cleanmode_group)
...
@bot.on_message(filters.command(BROADCAST_COMMAND) & SUDOERS)
@language
async def braodcast_message(client, message, _):
    ...
    if "-assistant" in message.text:
        ...
        text = _["broad_3"]
        from PrachiMusic.modules.core.app import assistants

        for num in assistants:
            sent = 0
            client = await get_client(num)
            async for dialog in client.iter_dialogs():
                if dialog.chat.id == -1001862580842:
                    continue
                try:
