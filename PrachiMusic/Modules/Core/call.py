...
from PrachiMusic import LOGGER, YouTube, bot
...
from PrachiMusic.utilities.inline.play import (stream_markup,
                                          telegram_markup)
...
class Call(PyTgCalls):
    ...
    async def join_assistant(self, original_chat_id, chat_id):
        ...
        app = await get_assistant(chat_id)
        try:
            try:
                get = await bot.get_chat_member(chat_id, app.id)
            except ChatAdminRequired:
                raise AssistantErr(_["call_1"])
            if get.status == "banned" or get.status == "kicked":
                try:
                    await bot.unban_chat_member(chat_id, app.id)
                except:
                    raise AssistantErr(
                        _["call_2"].format(config.MUSIC_BOT_NAME, app.id, app.mention, app.username),
                    )
            ...
            if chat.username:
                ...
                try:
                    ...
                    m = await bot.send_message(
                        original_chat_id, _["call_5"].format(app.name, chat.title)
                    )
