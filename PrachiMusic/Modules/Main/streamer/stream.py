...
from PrachiMusic.misc import db
from PrachiMusic.utilities import config
from PrachiMusic import Carbon, YouTube, bot
from PrachiMusic.modules.core.call import Kaal
from PrachiMusic.modules.main.database import (add_active_chat,
                                       add_active_video_chat,
                                       is_active_chat,
                                       is_video_allowed, music_on)
from PrachiMusic.modules.utils.exceptions import AssistantErr
from PrachiMusic.utilities.inline.play import (stream_markup, queue_markup,
                                          telegram_markup)
from PrachiMusic.utilities.inline.playlist import close_markup
from PrachiMusic.modules.main.streamer.queue import put_queue, put_queue_index
from PrachiMusic.modules.utils.pastebin import Kaalbin
from PrachiMusic.modules.utils.thumbnails import gen_thumb, que_thumb
...
async def stream(
    ...
    if forceplay:
        await Kaal.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        for search in result:
            ...
            if await is_active_chat(chat_id):
                ...
                run = await bot.send_photo(
                    original_chat_id,
                    photo=img,
                    caption=_["stream_1"].format(
                        title[:27],
                        f"https://t.me/{bot.username}?start=info_{vidid}",
                        duration_min,
                        user_name,
                    ),
        ...
        if await is_active_chat(chat_id):
            ...
            img = await gen_thumb(vidid, user_id)
            button = telegram_markup(_, vidid, chat_id)
            run = await bot.send_photo(
                original_chat_id,
                photo=img,
                caption=_["stream_1"].format(
